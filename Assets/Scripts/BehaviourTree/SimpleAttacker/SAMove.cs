...
namespace RTSZombie
{
	public class SAMove : Action
	{
		...
		public override void OnStart()
		{
			...
			currMoveCommand = owner.moveCommand;
		...
		{
			if(owner.moveCommand != currMoveCommand)
            {
				OnStart();
			}

			if (owner.friendlyUnitCollisions.Any(unit => {
				return unit.GetComponent<RZSimpleAttacker>().moveCommand == null &&
